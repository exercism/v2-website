...
class RetrieveSolutionsForMentorTest < ActiveSupport::TestCase
  test "retrieves solutions correctly" do
    ...
    stale = create :solution, last_updated_by_user_at: DateTime.now - 8.days
    ...
    completed_and_state = create :solution, completed_at: DateTime.now - 8.days, last_updated_by_user_at: DateTime.now - 8.days
    ...
    user = create :user
    [awaiting_user, completed, completed_and_state, stale].each do |solution|
    ...
    assert_equal [completed, completed_and_state], RetrieveSolutionsForMentor.retrieve(user, :completed)
    assert_equal [stale], RetrieveSolutionsForMentor.retrieve(user, :stale)
    assert_equal [abandoned_and_awaiting_user, abandoned_and_requires_action, abandoned_and_completed, abandoned_and_stale].sort, RetrieveSolutionsForMentor.retrieve(user, :unsubscribed).sort
