...
module Flux
  class MergeJSandECMATest < ActiveSupport::TestCase
    ...
    test "FixUnlockingInUserTrack is called for affected users" do
      ...
      FixUnlockingInUserTrack.expects(:call).with(ut2)
      FixUnlockingInUserTrack.expects(:call).with(ut3)

      # This shouldn't be called for the two js tracks
    ...
    test "create ecma user track and removes js user tracks" do
      ...
      mentored_js_track = create :user_track, user: mentored_js_user, track: @js, independent_mode: true
      independent_js_track = create :user_track, user: independent_js_user, track: @js, independent_mode: false
      ...
      [mentored_js_track, independent_js_track].reload
      ...
      assert_equal [mentored_js_user, independent_js_user, ecma_user, both_user].map(&:id), UserTrack.where(track: @ecma).pluck(:user_id)
      ...
      assert_equal UserTrack.find_by(track: @js, user: mentored_js_user).independent_mode
      refute UserTrack.find_by(track: @js, user: independent_js_user).independent_mode
    end
    ...
    test "create ecma track_mentorship" do
      ...
      assert_equal [js_mentor, ecma_mentor, both_mentor].map(&:id), TrackMentorship.where(track: @ecma).pluck(:user_id)
    ...
    test "renames the exercises correctly" do
      MergeJSAndECMA.()
    ...
    test "exercises are migrated to legacy exercise if there is a clash and ecma is approved" do
      js_bob_solution = create_solution(@js_bob, :started)
      ecma_bob_solution = create_solution(@ecma_bob, :approved)

      MergeJSAndECMA.()
      js_bob_solution.reload

      assert_equal @js_bob, js_bob_solution.exercise
    end

    test "exercises are migrated to legacy exercise if there is a clash and ecma is completed" do
      js_bob_solution = create_solution(@js_bob, :started)
      ecma_bob_solution = create_solution(@ecma_bob, :completed)
    ...
    test "exercises are migrated to ecma exercise if there is a clash and js is completed but ecma is started" do
      js_bob_solution = create_solution(@js_bob, :completed)
      ecma_bob_solution = create_solution(@ecma_bob, :started)
    ...
    test "exercises are migrated to ecma exercise if there is a clash and js is approved but ecma is started" do
      js_bob_solution = create_solution(@js_bob, :approved)
      ecma_bob_solution = create_solution(@ecma_bob, :started)
