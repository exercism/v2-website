...
class FixUnlockingInUserTrack
  ...
  def call
    ...
    completed_exercise_ids = Exercise.core.where(id: user_track.solutions.completed.map(&:exercise_id)).pluck(:id)
    ...
    existing_solution_ids, existing_exercise_ids = Solution.where(user_id: user.id).
                                                            joins(:exercise).
                                                            where("exercises.unlocked_by_id": completed_exercise_ids).
    ...
    # Make any new ones
    keep_solution_ids += Exercise.where(unlocked_by_id: completed_exercise_ids).
